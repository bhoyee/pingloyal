...
// Each rule: after stripping country code, the remaining subscriber number
// has exactly `subscriberLength` digits (no leading prefix).
// `localPrefix` is the trunk prefix (e.g. '0') used in local-format inputs.
interface CountryRule {
  countryCode: string;
  localPrefix: string;
  /** Total local-format digits including the trunk prefix */
  localTotalLength: number;
}
...
export function normalisePhone(raw: string): string {
  ...
  if (!cleaned) throw new PhoneNormalisationError(raw);

  // Already valid E.164 (+<7-15 digits>)
  if (/^\+\d{7,15}$/.test(cleaned)) return cleaned;

  // From here we only handle digit-only strings
  if (!/^\d+$/.test(cleaned)) throw new PhoneNormalisationError(raw);
  ...
  for (const rule of COUNTRY_RULES) {
    ...
    if (
      cleaned.startsWith(rule.countryCode) &&
