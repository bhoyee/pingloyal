...
async function main() {
  ...
  await SeedDataSource.transaction(async (manager) => {
    ...
    console.log(
      `[seed:recon] pointsEarnRate = ${String(tenant.pointsEarnRate)}`,
    );
    ...
    async function findOrCreate(
      ...
      let u = await manager.findOne(User, {
    ...
    const kemi = await findOrCreate(
      'kemi@jadefystore.com',
      'Kemi Adewale',
      UserRole.CASHIER,
    );
    const emeka = await findOrCreate(
      'emeka@jadefystore.com',
      'Emeka Nwosu',
      UserRole.CASHIER,
    );
    ...
    for (const [key, phone] of Object.entries(PHONES)) {
      const c = await manager.findOne(Customer, {
        where: { tenantId: tenant.id, phoneE164: phone },
      });
      if (!c)
        throw new Error(
          `Customer ${key} not found — run seed-jadefy.ts first.`,
        );
      custId[key] = c.id;
    }

    // ── 4. Find categories ────────────────────────────────────────────────────
    const cats = await manager.find(ProductCategory, {
    ...
    if (!catId['food'])
      throw new Error('No categories found — run seed-jadefy.ts first.');
    ...
    console.log('[seed:recon] Removed old reports-seed and recon-seed rows');
    ...
    const discPct =
      expectedPoints > 0 ? Math.round((discrepancy / expectedPoints) * 100) : 0;

    console.log(`\n[seed:recon] ✓ Inserted ${inserted} transactions`);
    console.log(
      `[seed:recon] Total spend:    ₦${totalAmount.toLocaleString()}`,
    );
    console.log(
      `[seed:recon] Expected pts:   ${expectedPoints.toLocaleString()}`,
    );
    console.log(`[seed:recon] Actual pts:     ${totalPoints.toLocaleString()}`);
    console.log(
      `[seed:recon] Discrepancy:    +${discrepancy.toLocaleString()} pts (${discPct}%)`,
    );

    console.log('\n[seed:recon] Expected Reconciliation page behaviour:');
    console.log(
      '[seed:recon]   ┌── Today      → ✓ GREEN  card  (0% discrepancy, 4 tx)',
    );
    console.log(
      '[seed:recon]   ├── Last 7 days → 🔴 RED   card  (~15% discrepancy)',
    );
    console.log(
      '[seed:recon]   └── Last 30 days→ 🔴 RED   card  (~15% discrepancy)',
    );
    ...
    console.log(
      '[seed:recon]     Emeka  → ⚠ Review  (avg ₦1,083, 100% manual, pts inflated)',
    );
    console.log('[seed:recon]     System → ✓ Normal  (webhook + file_import)');
    console.log(
      '[seed:recon]   Source breakdown: 80% manual · 20% terminal-verified',
    );
    ...
    console.log('\n[seed:recon] Removed old redemption rows');
    ...
    const redemptionRows: RedSpec[] = [
      ...
      {
        customerId: custId.adaeze,
        cashierId: kemi.id,
        rewardsCount: 1,
        pointsRedeemed: THRESHOLD,
        value: REWARD_VAL,
        balanceAfter: 200,
        redeemedAt: at(0, 10, 15),
      },
      {
        customerId: custId.funke,
        cashierId: owner.id,
        rewardsCount: 1,
        pointsRedeemed: THRESHOLD,
        value: REWARD_VAL,
        balanceAfter: 500,
        redeemedAt: at(0, 13, 45),
      },

      // ── LAST 7 DAYS (also appear on "Last 7 days") ───────────────────────
      {
        customerId: custId.chidinma,
        cashierId: kemi.id,
        rewardsCount: 1,
        pointsRedeemed: THRESHOLD,
        value: REWARD_VAL,
        balanceAfter: 1200,
        redeemedAt: at(2, 11, 30),
      },
      // Ibrahim processed by Emeka — adds context to his flagged status
      {
        customerId: custId.ibrahim,
        cashierId: emeka.id,
        rewardsCount: 1,
        pointsRedeemed: THRESHOLD,
        value: REWARD_VAL,
        balanceAfter: 100,
        redeemedAt: at(5, 14, 0),
      },

      // ── OLDER (visible on "Last 30 days") ────────────────────────────────
      {
        customerId: custId.tunde,
        cashierId: kemi.id,
        rewardsCount: 2,
        pointsRedeemed: 2 * THRESHOLD,
        value: 2 * REWARD_VAL,
        balanceAfter: 800,
        redeemedAt: at(10, 12, 0),
      },
      {
        customerId: custId.adaeze,
        cashierId: owner.id,
        ...
        value: REWARD_VAL,
        balanceAfter: 400,
        redeemedAt: at(15, 10, 0),
      },
      {
        customerId: custId.grace,
        cashierId: kemi.id,
        rewardsCount: 1,
        pointsRedeemed: THRESHOLD,
        value: REWARD_VAL,
        balanceAfter: 300,
        redeemedAt: at(20, 15, 30),
      },
      {
        customerId: custId.chidinma,
        cashierId: owner.id,
    ...
    const totalRedCount = redemptionRows.reduce(
      (s, r) => s + r.rewardsCount,
      0,
    );
    console.log(
      `[seed:recon] ✓ Inserted ${redemptionRows.length} redemption records`,
    );
    console.log(
      `[seed:recon] Total rewards given: ${totalRedCount}  ·  Value: ₦${totalRedValue.toLocaleString()}`,
    );
    console.log('[seed:recon] Redemptions by period:');
    console.log(
      '[seed:recon]   Today       → 2 redemptions (Adaeze + Funke with Kemi & Owner)',
    );
    console.log(
      '[seed:recon]   Last 7 days → 4 redemptions (+Chidinma, Ibrahim/Emeka)',
    );
    console.log(
      '[seed:recon]   Last 30 days→ 8 redemptions (10 rewards total, ₦10,000 value)',
    );
  });
