...
async function main() {
  ...
  if (!tid) { console.log('NO TENANT FOUND'); await ds.destroy(); return; }

  const txnCount = await ds.query(
    'SELECT COUNT(*) as cnt FROM transactions WHERE tenant_id = $1', [tid],
  ) as Array<{ cnt: string }>;
  console.log('Total transactions:', txnCount[0].cnt);

  const catTxns = await ds.query(
    'SELECT COUNT(*) as cnt FROM transactions WHERE tenant_id = $1 AND category_id IS NOT NULL', [tid],
  ) as Array<{ cnt: string }>;
  ...
  const cats = await ds.query(
    `SELECT pc.name, COUNT(*) as cnt
     ...
     WHERE t.tenant_id = $1
     GROUP BY pc.name ORDER BY cnt DESC`, [tid],
  );
  ...
  const dow = await ds.query(
    `SELECT EXTRACT(DOW FROM created_at) AS dow,
     ...
     GROUP BY dow, day_name ORDER BY dow`, [tid],
  );
  console.log('DOW breakdown:', JSON.stringify(dow));

  const snap = await ds.query(
    'SELECT period_type, period_start, period_end, computed_at FROM report_snapshots WHERE tenant_id = $1', [tid],
  );
