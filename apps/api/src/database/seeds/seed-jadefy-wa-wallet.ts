...
async function main() {
  ...
  await ds.transaction(async (mgr) => {
    ...
    console.log(
      `[seed:wa-wallet] Customers: ${allCustomers.length} total, ${opted.length} opted-in`,
    );
    ...
    console.log(
      `[seed:wa-wallet] Purged ${String((wDel as number[])[1] ?? 0)} wallet txns, ${String((tDel as number[])[1] ?? 0)} trigger logs`,
    );
    ...
    const batches: TLogBatch[] = [
      ...
      {
        type: TriggerType.WALLET_LOW_BALANCE,
        daysAgo: 5,
        hour: 6,
        recipientIndexes: [0],
        deliveryRate: 1.0,
    ...
    for (const batch of batches) {
      for (const idx of batch.recipientIndexes) {
        ...
        const isDelivered =
          tlogSeq % 100 < Math.round(batch.deliveryRate * 100);
        const status = isDelivered
          ? TriggerStatus.DELIVERED
          : TriggerStatus.SENT;
        const createdAt = daysAgoAt(
          batch.daysAgo,
          batch.hour,
          (tlogSeq * 3) % 59,
        );
    ...
    console.log(
      `[seed:wa-wallet] Campaign msgs: ${campaignCount}, Lapsed: ${lapsedCount}, Birthday: ${birthdayCount}`,
    );
    ...
    for (const d of campaignBatchDays) {
      const batchMsgs = batches
        .filter(
          (b) => b.type === TriggerType.CAMPAIGN_MESSAGE && b.daysAgo === d,
        )
        .reduce((s, b) => s + b.recipientIndexes.length, 0);
      walletSpecs.push({
    ...
    let runningBalance =
      Math.round(tenant.marketingWalletBalance ?? 25000) + totalDebits;
    ...
    console.log(
      `[seed:wa-wallet] Inserted ${walletInserted} wallet transactions`,
    );
