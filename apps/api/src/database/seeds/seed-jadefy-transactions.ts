...
async function main() {
  ...
  await SeedDataSource.transaction(async (manager) => {
    ...
    console.log(
      `[seed:transactions] Tenant: ${tenant.id} (${tenant.businessName})`,
    );
    ...
    if (categories.length === 0) {
      throw new Error(
        'No product categories found — run seed-jadefy.ts first.',
      );
    }
    ...
    console.log(
      `[seed:transactions] Removed ${Array.isArray(deleteResult) ? String(deleteResult[1] ?? 0) : '?'} old seed transactions`,
    );
    ...
    for (const [name, count] of Object.entries(catCount).sort(
      (a, b) => b[1] - a[1],
    )) {
      console.log(
        `  ${name}: ${count} txns (${Math.round((count / txns.length) * 100)}%)`,
      );
    }
