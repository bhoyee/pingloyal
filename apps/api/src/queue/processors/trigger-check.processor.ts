...
export class TriggerCheckProcessor extends WorkerHost {
  ...
  async process(job: Job<TriggerCheckJobPayload>): Promise<void> {
    ...
    if (pct >= 0.8 && pct < 1.0 && isTriggerEnabled(tenant.enabledTriggers, TriggerType.THRESHOLD_NUDGE)) {
    ...
    if (pct >= 1.0 && isTriggerEnabled(tenant.enabledTriggers, TriggerType.REWARD_UNLOCKED)) {
